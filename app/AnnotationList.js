...
class AnnotationList extends React.Component {
  ...
  renderPaginator() {
    ...
    if (this.state.page === 1) {
      elements.push(
        <li key="page-back" className="page-item disabled">
      ...
      elements.push(
        <li key="page-back" className="page-item">
          <div
            className="page-link"
    ...
    for (let pageNum = 1; pageNum <= pages; pageNum++) {
      if (this.state.page === pageNum) {
        elements.push(
          <li key={`page-${pageNum}`} className="page-item active" aria-current="page">
    ...
    if (this.state.page === pages) {
      elements.push(
        <li key="page-foward" className="page-item disabled">
          <div className="page-link" tabIndex="-1" aria-disabled="true">
      ...
      elements.push(
        <li key="page-foward" className="page-item">
          <div
            className="page-link"
            onClick={() => this.changePage(this.state.page + 1)}
          >
  ...
  renderLabels(labels) {
    return labels.map((label) => {
      return (
        <span key={`item-label-${label.value}`} className="badge badge-dark m-1">{label.value}</span>
      )
    })
  }
  ...
  renderEmptyList() {
    return (
      <div className="d-flex flex-column justify-content-center w-100" style={{height: '20em'}}>
